...
<img src="<?= Html::encode($model->avatar); ?>.jpg" 
srcset="<?= Html::encode($model->avatar); ?>@2x.jpg 2x" alt="Автар <?= Html::encode($model->name); ?>">
</div>
<?php endif; ?>
</div>
...
->label(
    (isset($model->avatar)) ? 'Загрузить другое фото…' : 'Загрузить фото…',
    ['for' => 'button-input', 'class' => 'js-file-field']
); ?>
</div>
...
'inputOptions' => ['placeholder' => 'Электронная почта']])->input('email');
?>
...
'enableAjaxValidation' => true, 'template' => "{input}\n{error}", 'options' => [
'class' => 'form__field sign-up__field'],
'inputOptions' => ['placeholder' => 'Повтор пароля']])->passwordInput();
?>        
